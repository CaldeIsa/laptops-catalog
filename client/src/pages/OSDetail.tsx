    ...
    const fetchLaptops = async () => {
      try {
        ...
        const filtered = data.filter((laptop: Laptop) => {
          const laptopOSSlug = laptop.os
            .toLowerCase()
            .replace(/[^\w\s-]/g, "")
            .replace(/[\s_-]+/g, "-")
            .replace(/^-+|-+$/g, "");
          return laptopOSSlug === slug;
        });
        
        if (filtered.length > 0) {
          setOSName(filtered[0].os);
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <header className="bg-white shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Laptops con {osName}</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        {laptops.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-slate-600 text-lg">No se encontraron laptops para este sistema operativo.</p>
          </div>
        ) : (
