    ...
    const fetchLaptops = async () => {
      try {
        ...
        const filtered = data.filter((laptop: Laptop) => {
          const laptopProcSlug = laptop.processor
            .toLowerCase()
            .replace(/[^\w\s-]/g, "")
            .replace(/[\s_-]+/g, "-")
            .replace(/^-+|-+$/g, "");
          return laptopProcSlug === slug;
        });
        
        if (filtered.length > 0) {
          setProcessorName(filtered[0].processor);
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <header className="bg-white shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Laptops con {processorName}</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        {laptops.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-slate-600 text-lg">No se encontraron laptops con este procesador.</p>
          </div>
        ) : (
              ...
              <Card key={laptop.id} className="hover:shadow-lg transition-shadow border-0">
                ...
                <CardContent className="space-y-3">
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-600">Pantalla:</span>
                      <span className="font-medium">{laptop.screen_size} cm</span>
                    </div>
                    <div className="flex justify-between">
