    ...
    const fetchLaptops = async () => {
      try {
        ...
        const filtered = data.filter((laptop: Laptop) => 
          laptop.brand.toLowerCase() === slug?.toLowerCase()
        );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <header className="bg-white shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Laptops {brandName}</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        {laptops.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-slate-600 text-lg">No se encontraron laptops para esta marca.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {laptops.map((laptop) => (
              <Card key={laptop.id} className="hover:shadow-lg transition-shadow border-0">
                <CardHeader>
                  <CardTitle className="text-lg">{laptop.model}</CardTitle>
                </CardHeader>
